<?php
/**
 * Image rendering service for serving optimized formats.
 *
 * @package FluxMedia
 * @since 1.0.0
 */
...
class ImageRenderer {
    ...
    public function init() {
        // Hook into WordPress image rendering
        add_filter( 'wp_get_attachment_image_attributes', [ $this, 'modify_image_attributes' ], 10, 3 );
        add_filter( 'wp_content_img_tag', [ $this, 'modify_content_images' ], 10, 3 );
        add_filter( 'the_content', [ $this, 'modify_post_content_images' ], 20 );
        
        // Hook into attachment edit screen
    ...
    public function modify_image_attributes( $attr, $attachment, $size ) {
        // Check if we have converted files for this attachment
        if ( ! $this->image_converter->has_converted_files( $attachment->ID ) ) {
            return $attr;
        }

        $converted_files = $this->image_converter->get_converted_files( $attachment->ID );
     ...
     */
    public function modify_content_images( $filtered_image, $context, $attachment_id ) {
        // Check if we have converted files for this attachment
        if ( ! $this->image_converter->has_converted_files( $attachment_id ) ) {
            return $filtered_image;
        }

        $converted_files = $this->image_converter->get_converted_files( $attachment_id );
    ...
    public function modify_attachment_fields( $form_fields, $post ) {
        ...
        // Check if we have converted files (use appropriate converter)
        $converter = $is_image ? $this->image_converter : $this->video_converter;
        if ( ! $converter->has_converted_files( $post->ID ) ) {
            return $form_fields;
        }

        $converted_files = $converter->get_converted_files( $post->ID );
